from model.synthesizer.syn_SubLayer import Self_Attention
import torch.nn as nn
import torch
from syn_SubLayer import *
...
class syn_EncoderLayer(nn.Module):
    def __init__(self, config):
        super(syn_EncoderLayer, self).__init__()
        self.context_encoder = Self_Attention(config)
    ...
    def forward(self, src, mask, use_gpu):
        ...
        temp = self.context_encoder(src, mask, use_gpu)
        src = temp + src
...
class syn_Encoder(nn.Module):
    def __init__(self, config, src_embedding_num, embedding_matrix, embedding_dim_size):
        ...
        self.encoder_layer = get_clones(syn_EncoderLayer(config), self.layer_num)
